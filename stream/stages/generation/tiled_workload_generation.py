...
NODE_TILING_CONFIG_T = dict[int, dict[str, TILING_T | TILING_WILDCARD_T]]
TILING_CONFIGURATION_T = dict[str, Any]
...
class TiledWorkloadGenerationStage(Stage):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Optional list of tiling configurations to explore in one stage run.
        # Each element can define:
        # - name: str (optional)
        # - node_tilings: {node_id: {"intra_core_tiling": [...], "inter_core_tiling": [...]} }
        # - intra_core_tiling: {node_id: [...] } (optional shorthand)
        # - inter_core_tiling: {node_id: [...] } (optional shorthand)
        self.tiling_configurations: list[TILING_CONFIGURATION_T] = kwargs.get("tiling_configurations", [])
        # Optional automatic exploration of intra-core tilings.
        # When enabled, `tiling_configurations` can be omitted and configurations are generated by taking
        # prime-factor-based divisors of selected layer dimensions for each node, then evaluating all combinations.
        self.explore_intra_core_tiling: bool = kwargs.get("explore_intra_core_tiling", False)
        self.explore_intra_core_tiling_dims: list[str | LayerDim] = kwargs.get(
            "explore_intra_core_tiling_dims", [LayerDim("K"), LayerDim("OY")]
        )
        self.max_explored_tiling_configurations: int | None = kwargs.get("max_explored_tiling_configurations")
        # If True, run substages for each configuration instead of selecting only one.
        self.run_all_tiling_configurations: bool = kwargs.get("run_all_tiling_configurations", False)
        # Auto exploration is intended to evaluate all generated combinations.
        if self.explore_intra_core_tiling:
    ...
    def run(self):
        ...
        kwargs["tiled_workloads"] = tiled_workloads
        kwargs["tiling_configuration_names"] = list(tiled_workloads.keys())

        if self.run_all_tiling_configurations:
            for config_name, _, original_workload, tiled_workload in workloads_per_config:
                config_kwargs = kwargs.copy()
                config_kwargs["tiling_configuration_name"] = config_name
                config_kwargs["original_workload"] = pickle_deepcopy(original_workload)
                config_kwargs["workload"] = tiled_workload
                if "scheduling_order" not in config_kwargs:
                    config_kwargs["scheduling_order"] = self.get_scheduling_order(tiled_workload)
                sub_stage = self.list_of_callables[0](self.list_of_callables[1:], **config_kwargs)
                yield from sub_stage.run()
            yield None, None
            return

        if self.selected_tiling_configuration is None:
            selected_config = workloads_per_config[0][0]
        else:
            selected_config = self.selected_tiling_configuration
            if selected_config not in tiled_workloads:
                raise ValueError(
                    f"selected_tiling_configuration='{selected_config}' is not in {list(tiled_workloads.keys())}."
                )

        kwargs["tiling_configuration_name"] = selected_config
        kwargs["original_workload"] = pickle_deepcopy(original_workloads[selected_config])
        kwargs["workload"] = tiled_workloads[selected_config]
        if "scheduling_order" not in kwargs:
            kwargs["scheduling_order"] = self.get_scheduling_order(tiled_workloads[selected_config])
        sub_stage = self.list_of_callables[0](self.list_of_callables[1:], **kwargs)
        yield from sub_stage.run()

        yield None, None

    def generate_tiled_workloads_for_configurations(
        self,
    ) -> list[tuple[str, NODE_TILING_CONFIG_T, ONNXWorkload, ComputationNodeWorkload]]:
        """Generate tiled workloads for either:
        - one implicit 'default' configuration (legacy behavior), or
        - multiple user-provided tiling configurations."""
    ...
    @staticmethod
    def _cluster_key(node: ComputationNode) -> int:
        """Cluster proxy used for locality/traffic metrics.
        Prefer chosen core allocation when available, otherwise fall back to layer id.
        """
        chosen = getattr(node, "chosen_core_allocation", None)
        return int(chosen) if chosen is not None else int(node.id)

    @classmethod
    def _coefficient_of_variation(cls, values: list[float]) -> float:
        if not values:
            return 0.0
        mean_value = cls._safe_div(sum(values), float(len(values)))
        if mean_value <= 0.0:
            return 0.0
        variance = sum((v - mean_value) ** 2 for v in values) / float(len(values))
        std_dev = variance**0.5
        return cls._safe_div(std_dev, mean_value)

    @classmethod
    def _compute_graph_metrics(cls, workload: ComputationNodeWorkload) -> dict[str, float]:
        edges = list(workload.edges(data=True))
        nb_nodes = float(workload.number_of_nodes())
        nb_edges_all = float(len(edges))
        nb_edges = 0.0
        total_bits = 0.0
        comm_edges = 0.0
        cross_layer_edges = 0.0
        cross_layer_bits = 0.0
        intra_cluster_edges = 0.0
        intra_cluster_bits = 0.0
        inter_cluster_edges = 0.0
        inter_cluster_bits = 0.0
        inter_cluster_out_bits_per_src: dict[int, float] = defaultdict(float)
        inter_cluster_in_bits_per_dst: dict[int, float] = defaultdict(float)
        ...
        for producer, consumer, data in edges:
            ...
            if producer.id != consumer.id:
                cross_layer_edges += 1.0
                cross_layer_bits += bits
            src_cluster = cls._cluster_key(producer)
            dst_cluster = cls._cluster_key(consumer)
            if src_cluster == dst_cluster:
                intra_cluster_edges += 1.0
                intra_cluster_bits += bits
            else:
                inter_cluster_edges += 1.0
                inter_cluster_bits += bits
                inter_cluster_out_bits_per_src[src_cluster] += bits
                inter_cluster_in_bits_per_dst[dst_cluster] += bits

        bits_exchanged_per_edge = cls._safe_div(total_bits, nb_edges)
        comm_edge_ratio = cls._safe_div(comm_edges, nb_edges_all)
        cross_layer_edge_ratio = cls._safe_div(cross_layer_edges, nb_edges)
        cross_layer_bits_per_edge = cls._safe_div(cross_layer_bits, cross_layer_edges)
        edges_per_node = cls._safe_div(nb_edges, nb_nodes)
        intra_cluster_edge_ratio = cls._safe_div(intra_cluster_edges, nb_edges)
        intra_cluster_bits_ratio = cls._safe_div(intra_cluster_bits, total_bits)
        inter_cluster_bits_ratio = cls._safe_div(inter_cluster_bits, total_bits)
        ...
        memory_pressure_peak_over_mean = cls._safe_div(max_dst_pressure, mean_dst_pressure)
        memory_pressure_risk_excess = max(0.0, memory_pressure_peak_over_mean - 1.0)
        output_tile_sizes_bits = [int(node.get_output_tensor().size) for node in workload.node_list]
        per_output_tile_required_intermediate_bits: list[int] = []
        for node in workload.node_list:
            required_bits = 0
            for _, _, edge_data in workload.in_edges(node, data=True):
                bits_raw = edge_data.get("bits", 0) if isinstance(edge_data, dict) else 0
                bits = int(bits_raw)
                if bits > 0:
                    required_bits += bits
            per_output_tile_required_intermediate_bits.append(required_bits)
        min_output_tile_size_bits = min(output_tile_sizes_bits) if output_tile_sizes_bits else 0
        max_output_tile_size_bits = max(output_tile_sizes_bits) if output_tile_sizes_bits else 0
        avg_output_tile_size_bits = cls._safe_div(
            float(sum(output_tile_sizes_bits)), float(len(output_tile_sizes_bits))
        )
        min_required_intermediate_bits_per_output_tile = (
            min(per_output_tile_required_intermediate_bits) if per_output_tile_required_intermediate_bits else 0
        )
        max_required_intermediate_bits_per_output_tile = (
            max(per_output_tile_required_intermediate_bits) if per_output_tile_required_intermediate_bits else 0
        )
        avg_required_intermediate_bits_per_output_tile = cls._safe_div(
            float(sum(per_output_tile_required_intermediate_bits)),
            float(len(per_output_tile_required_intermediate_bits)),
        )

        # Lower is better for fusion friendliness.
        fusion_proxy_score = (
            2.0 * cross_layer_edges
            + 1.0 * comm_edges
            + 1e-6 * cross_layer_bits
            + 1e-6 * total_bits
        )
        # Higher is better:
        # maximize intra-cluster locality, minimize inter-cluster traffic, imbalance and pressure hotspots.
        fusion_locality_tradeoff_score = cls._safe_div(
            intra_cluster_bits_ratio,
            1.0 + inter_cluster_bits_ratio + inter_cluster_traffic_imbalance_cv + memory_pressure_risk_excess,
        )

        return {
            "number_of_nodes": nb_nodes,
            "number_of_edges": nb_edges,
            "total_bits_exchanged": total_bits,
            "bits_exchanged_per_edge": bits_exchanged_per_edge,
            "communication_edges": comm_edges,
            "communication_edge_ratio": comm_edge_ratio,
            "cross_layer_edges": cross_layer_edges,
            "cross_layer_edge_ratio": cross_layer_edge_ratio,
            "cross_layer_bits_exchanged": cross_layer_bits,
            "cross_layer_bits_per_edge": cross_layer_bits_per_edge,
            "edges_per_node": edges_per_node,
            "fusion_proxy_score": fusion_proxy_score,
            "intra_cluster_edges": intra_cluster_edges,
            "inter_cluster_edges": inter_cluster_edges,
            "intra_cluster_bits_exchanged": intra_cluster_bits,
            "inter_cluster_bits_exchanged": inter_cluster_bits,
            "intra_cluster_edge_ratio": intra_cluster_edge_ratio,
            ...
            "min_output_tile_size_bits": float(min_output_tile_size_bits),
            "max_output_tile_size_bits": float(max_output_tile_size_bits),
            "avg_output_tile_size_bits": avg_output_tile_size_bits,
            "min_required_intermediate_bits_per_output_tile": float(
                min_required_intermediate_bits_per_output_tile
            ),
            "max_required_intermediate_bits_per_output_tile": float(
                max_required_intermediate_bits_per_output_tile
            ),
            "avg_required_intermediate_bits_per_output_tile": avg_required_intermediate_bits_per_output_tile,
            "output_tile_sizes_bits": json.dumps(output_tile_sizes_bits),
            "required_intermediate_bits_per_output_tile": json.dumps(
                per_output_tile_required_intermediate_bits
            ),
        }
    ...
    @classmethod
    def _serialize_node_tilings(cls, node_tilings: NODE_TILING_CONFIG_T) -> dict[str, dict[str, Any]]:
        serialized: dict[str, dict[str, Any]] = {}
        for node_id, config in sorted(node_tilings.items()):
            node_key = str(node_id)
            serialized[node_key] = {}
            if "intra_core_tiling" in config:
                serialized[node_key]["intra_core_tiling"] = cls._serialize_tiling(
                    config["intra_core_tiling"]  # type: ignore[arg-type]
                )
            if "inter_core_tiling" in config:
                serialized[node_key]["inter_core_tiling"] = cls._serialize_tiling(
                    config["inter_core_tiling"]  # type: ignore[arg-type]
                )
        return serialized

    def _normalize_tiling_configurations(self) -> list[tuple[str, NODE_TILING_CONFIG_T]]:
        if self.explore_intra_core_tiling:
            return self._generate_tiling_configurations_from_prime_factors()

        if not self.tiling_configurations:
            return [("default", {})]

        normalized_configs: list[tuple[str, NODE_TILING_CONFIG_T]] = []
        seen_names: set[str] = set()
        for i, config in enumerate(self.tiling_configurations):
            config_name = str(config.get("name", f"config_{i}"))
            if config_name in seen_names:
                raise ValueError(f"Duplicate tiling configuration name '{config_name}'. Names must be unique.")
            seen_names.add(config_name)
            node_tilings: NODE_TILING_CONFIG_T = deepcopy(config.get("node_tilings", {}))
            intra_tilings: dict[int, TILING_T] = config.get("intra_core_tiling", {})
            inter_tilings: dict[int, TILING_WILDCARD_T | TILING_T] = config.get("inter_core_tiling", {})

            for node_id, intra_tiling in intra_tilings.items():
                node_tilings.setdefault(node_id, {})["intra_core_tiling"] = deepcopy(intra_tiling)
            for node_id, inter_tiling in inter_tilings.items():
                node_tilings.setdefault(node_id, {})["inter_core_tiling"] = deepcopy(inter_tiling)

            normalized_configs.append((config_name, node_tilings))

        return normalized_configs

    def _generate_tiling_configurations_from_prime_factors(self) -> list[tuple[str, NODE_TILING_CONFIG_T]]:
        """Automatically generate tiling configurations from divisors of selected dimensions.
        For each node and selected dim (e.g., K/OY), candidate tiling sizes are all divisors of that dim size.
        A divisor of 1 means "do not tile this dim" and is omitted from the resulting intra_core_tiling list.
        All per-node candidate tilings are then combined with a Cartesian product across nodes.
        """
        explored_dims = [LayerDim(str(dim)) for dim in self.explore_intra_core_tiling_dims]
        tiling_nodes = [n for n in self.workload.node_list if isinstance(n, ComputationNode)]

        per_node_candidates: list[tuple[int, list[TILING_T]]] = []
        for node in tiling_nodes:
            dims_to_explore = [dim for dim in explored_dims if dim in node.layer_dim_sizes]
            if not dims_to_explore:
                per_node_candidates.append((node.id, [deepcopy(node.intra_core_tiling)]))
                continue

            dim_divisors: list[list[int]] = [
                self._get_all_divisors(int(node.layer_dim_sizes[dim])) for dim in dims_to_explore
            ]
            node_tiling_candidates: list[TILING_T] = []
            for chosen_sizes in product(*dim_divisors):
                candidate: TILING_T = []
                for dim, size in zip(dims_to_explore, chosen_sizes, strict=False):
                    if size > 1:
                        candidate.append((dim, size))
                node_tiling_candidates.append(candidate)

            per_node_candidates.append((node.id, node_tiling_candidates))

        configuration_space_size = (
            prod(len(candidates) for _, candidates in per_node_candidates) if per_node_candidates else 1
        )
        # if self.max_explored_tiling_configurations is not None and configuration_space_size > self.max_explored_tiling_configurations:
        #     raise ValueError(
        #         "Generated tiling configuration space too large: "
        #         f"{configuration_space_size} > max_explored_tiling_configurations={self.max_explored_tiling_configurations}."
        #     )

        logger.info(
            "Exploring %d intra-core tiling configuration(s) from prime-factor-based divisors.",
            configuration_space_size,
        )

        if not per_node_candidates:
            return [("auto_config_0", {})]

        node_ids = [node_id for node_id, _ in per_node_candidates]
        candidate_lists = [candidates for _, candidates in per_node_candidates]
        normalized_configs: list[tuple[str, NODE_TILING_CONFIG_T]] = []
        for idx, node_choices in enumerate(product(*candidate_lists)):
            node_tilings: NODE_TILING_CONFIG_T = {}
            for node_id, intra_tiling in zip(node_ids, node_choices, strict=False):
                node_tilings[node_id] = {"intra_core_tiling": deepcopy(intra_tiling)}
            normalized_configs.append((f"auto_config_{idx}", node_tilings))
    ...
    @classmethod
    def _get_all_divisors(cls, n: int) -> list[int]:
        """Return sorted divisors of n, including 1 and n, generated from prime factors."""
        prime_factors = cls._get_prime_factors(n)
        divisors = {1}
        for factor in prime_factors:
            divisors |= {d * factor for d in divisors}
        return sorted(divisors)

    def _apply_node_tilings(self, workload: ONNXWorkload, node_tilings: NODE_TILING_CONFIG_T):
        if not node_tilings:
            return
        tiling_nodes = [n for n in workload.node_list if isinstance(n, ComputationNode)]
        nodes_by_id: dict[int, list[ComputationNode]] = defaultdict(list)
        for node in tiling_nodes:
            nodes_by_id[node.id].append(node)
        for node_id, tilings in node_tilings.items():
            if node_id not in nodes_by_id:
                raise ValueError(f"Unable to apply tiling config: node id {node_id} does not exist in workload.")
            for node in nodes_by_id[node_id]:
                if "intra_core_tiling" in tilings:
                    node.intra_core_tiling = deepcopy(tilings["intra_core_tiling"])  # type: ignore[assignment]
                if "inter_core_tiling" in tilings:
                    node.inter_core_tiling = deepcopy(tilings["inter_core_tiling"])  # type: ignore[assignment]

    def _get_tiled_workload_path_for_config(self, config_name: str) -> str:
        if config_name == "default":
            return self.tiled_workload_path
        base, ext = os.path.splitext(self.tiled_workload_path)
        safe_name = re.sub(r"[^A-Za-z0-9_.-]+", "_", config_name.strip()) or "config"
        return f"{base}.{safe_name}{ext}"

    def _generate_tiled_workload_for_workload(
        self, workload: ONNXWorkload, tiled_workload_path: str
        ...
        cached_workload = self.load_cached_tiled_workload(tiled_workload_path)
        if cached_workload and self.cached_workload_matches(all_tiles, cached_workload):
            ...
            pickle_save(tiled_workload, tiled_workload_path)  # type: ignore
            logger.info(f"Saved tiled workload to {tiled_workload_path}.")
    ...
    def load_cached_tiled_workload(self, tiled_workload_path: str | None = None) -> ComputationNodeWorkload | None:
        path = tiled_workload_path if tiled_workload_path is not None else self.tiled_workload_path
