...
class PoolingGradParser(OnnxComputeOperatorParser):
    """Parses an onnx pooling gradient operator into a PoolingNode.
    e.g. MaxPoolGrad, AveragePoolGrad, etc.
    """

    def get_kernel_shape(self, attrs: dict, ia_dimension_shape: list[int]) -> list[int]:
        """Return the kernel shape of the pooling gradient operator.
        Args:
            attrs: Node attributes.
            ia_dimension_shape: Input activation dimension shape.
        """
        kernel_shape: list[int] | int = []
        if self.node.op_type in ["MaxPoolGrad", "AveragePoolGrad"]:
            # Find kernel shape in attrs
            kernel_shape = get_attribute_ints_with_name("kernel_shape", attrs, default=None)
        elif self.node.op_type in ["GlobalMaxPoolGrad", "GlobalAveragePoolGrad"]:
            ...
            raise NotImplementedError(
                f"Pooling gradient node kernel shape extraction not implemented for operand type {self.node.op_type}."
            )
    ...
    def get_layer_node_user_format(
        ...
        """
        Generate the necessary dictionary items required for the LayerNode creation for the pooling gradient.
        For the pooling gradient node, the equation and loop dimensions are adjusted for the backward pass.
        """
        data: dict[str, Any] = {}
        data["id"] = self.node_id
        data["name"] = self.node.name
        data["operator_type"] = self.node.op_type
        # Equation for average pooling gradient: distribute the gradient equally to all elements in the pooling window
        ...
        FX = kernel_shape[0]
        FY = kernel_shape[1]
        assert K == C, f"Input and output channels not equal for pooling gradient node {self.node.name}."
        data["loop_dims"] = ["B", "K", "IX", "IY", "C", "FX", "FY"]
        data["loop_sizes"] = [B, K, IX, IY, C, FX, FY]
        ...
        assert len(preds) <= 1
        if len(preds) == 1:
            data["operand_source"]["I"] = preds[0]  # Output gradient is the input to this node
